...
public class Pager<I, O> {
   ...
   */
  @SuppressWarnings("unchecked") public static <T> Observable<T> finish() {
   ...
   * @return a new sequence based on {@code source}, where subscribers keep receiving pages through
   * subsequent calls
   * to {@link #next()}
   */
  public Observable<O> page(final Observable<I> source) {
    return Observable.create(new Observable.OnSubscribe<O>() {
      @Override public void call(final Subscriber<? super O> subscriber) {
  ...
  /**
   * Advances the pager by pushing the next page of items into the current observer, is there is
   * one. If the pager
  ...
  private final class PageSubscriber extends Subscriber<I> {
    ...
    @Override public void onNext(I result) {
