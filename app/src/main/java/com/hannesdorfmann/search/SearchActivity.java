...
import com.hannesdorfmann.InfiniteScrollListener;
...
public class SearchActivity extends MvpViewStateActivity<SearchView, SearchPresenter>
  ...
  @Bind(R.id.search_view) android.widget.SearchView searchView;
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    AnimatedVectorDrawable searchToBack =
        (AnimatedVectorDrawable) ContextCompat.getDrawable(this, R.drawable.avd_search_to_back);
    ...
    scrim.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
      @Override public boolean onPreDraw() {
        ...
        showScrim.playTogether(ViewAnimationUtils.createCircularReveal(scrim, searchIconCenterX,
                searchBackground.getBottom(), 0, (float) Math.hypot(searchBackDistanceX,
  ...
  @OnClick({ R.id.scrim, R.id.searchback }) protected void dismiss() {
    ...
    AnimatedVectorDrawable backToSearch =
        (AnimatedVectorDrawable) ContextCompat.getDrawable(this, R.drawable.avd_back_to_search);
    ...
    if (resultsContainer.getHeight() > 0) {
      Animator closeResults =
          ViewAnimationUtils.createCircularReveal(resultsContainer, searchIconCenterX, 0,
  ...
  @OnClick(R.id.fab) protected void save() {
    ...
    confirmSaveContainer.getViewTreeObserver()
        .addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
          @Override public boolean onPreDraw() {
            ...
            Animator revealScrim =
                ViewAnimationUtils.createCircularReveal(resultsScrim, centerX, centerY, 0,
  ...
  @OnClick(R.id.save_confirmed) protected void doSave() {
    Intent saveData = new Intent();
    saveData.putExtra(EXTRA_QUERY, searchView.getQuery());
  ...
  @OnClick(R.id.results_scrim) protected void hideSaveConfimation() {
    if (confirmSaveContainer.getVisibility() == View.VISIBLE) {
      // contract the bubble & hide the scrim
      AnimatorSet hideConfirmation = new AnimatorSet();
      hideConfirmation.playTogether(ViewAnimationUtils.createCircularReveal(confirmSaveContainer,
              confirmSaveContainer.getWidth() / 2, confirmSaveContainer.getHeight() / 2,
  ...
  private void setNoResultsVisibility(int visibility) {
    if (visibility == View.VISIBLE) {
      ...
      String message =
          String.format(getString(R.string.no_search_results), searchView.getQuery().toString());
  ...
  @Override public void showLoadingMore(boolean loading) {
    castedViewState().setShowLoadingMore(loading);
    adapter.setLoadingMore(loading);
    adapter.notifyDataSetChanged();
  }

  @Override public void showLoadingMoreError(Throwable t) {
    Toast.makeText(this, R.string.error_more_search_results, Toast.LENGTH_SHORT).show();
  }
  ...
  @Override public void showError(@NotNull Throwable t) {
  ...
  @Override public void showSearchNotStarted() {
    castedViewState().setShowSearchNotStarted();
    clearResults();
  }

  private SearchViewState castedViewState() {
    return (SearchViewState) viewState;
  }

  @Override public ViewState<SearchView> createViewState() {
  ...
  private ObjectGraph getObjectGraph() {
    // TODO testing
    return PlaidApplication.getObjectGraph(this).plus(new SearchModule());
  }
}
